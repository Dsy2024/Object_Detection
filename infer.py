...
def crop(img="images/119.jpg"):
    ...
    return dict(zip(map(int, final_x), map(int, DEFAULT_FREQUENCIES))), final_y[-1]
...
if __name__ == "__main__":
    for i in range(0, 1):
        ...
        img_path = f"images/{i}.jpg" # Input image path
        model_path = "outputs/yolo11n_detect_OX/weights/best.pt"  # Trained model path
        save_name = f"{i}.jpg"  # Output image name
        
        px_to_hz, y_ratio = crop(img=img_path)
        results = yolo_infer(model_path=model_path, img="cropped_audiogram_v2.png", save_name=save_name)

        rows = []

        for r in results:
            for b in r.boxes:
                x_center = b.xywh[0, 0].item()  # x
                y_center = b.xywh[0, 1].item()  # y
                cls = int(b.cls.item())
                conf = float(b.conf.item()) if hasattr(b, "conf") else np.nan

                k = min(px_to_hz.keys(), key=lambda p: abs(p - x_center))
                nearest_hz = px_to_hz[k]
                db_level = -20 + round((y_center / y_ratio) * 140)  # Scale to -20 - 120 dB
                # print(
                #     f"Detected class {cls} at (x={round(x_center)}, y={round(y_center)}) → Frequency: {nearest_hz} Hz, dB: {db_level}"
                # )

                rows.append({"cls": cls, "hz": nearest_hz, "db": db_level, "conf": conf})

        df = pd.DataFrame(rows)
        # if "conf" in df.columns:
        #     df = df.sort_values("conf", ascending=False).drop_duplicates(subset=["cls", "hz"])

        wide = df.pivot(index="cls", columns="hz", values="db")
        wide = wide.reindex(columns=DEFAULT_FREQUENCIES).sort_index()

        wide.to_csv(f"outputs/csv/symbols_db_by_class_freq_{i}.csv", index_label="cls", float_format="%.2f") # Output CSV path
        print(f"Saved → symbols_db_by_class_freq_{i}.csv")